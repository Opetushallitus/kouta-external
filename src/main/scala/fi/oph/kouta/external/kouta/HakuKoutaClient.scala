...
object HakuKoutaClient extends HakuKoutaClient(CasKoutaClient)

class HakuKoutaClient(val client: CasKoutaClient) {

  def createHaku(request: KoutaHakuRequest): Future[KoutaResponse[HakuOid]] = {
    val url = client.urlProperties.url("kouta-backend.haku")
  ...
  def updateHaku(request: KoutaHakuRequest, ifUnmodifiedSince: Instant): Future[KoutaResponse[UpdateResponse]] = {
    val url = client.urlProperties.url("kouta-backend.haku")
    client.update(url, request, ifUnmodifiedSince)
  }
}
