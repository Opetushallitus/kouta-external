...
class HakukohdeClient(val client: ElasticClient, val clientJava: elasticsearch.ElasticsearchClient)
  ...
  def search(searchParams: HakukohdeSearchParams, hakukohdeOids: Option[Set[HakukohdeOid]]): Future[Seq[Hakukohde]] = {
    val hakuQuery =
      searchParams.hakuOid.map(oid => TermQuery.of(m => m.field("hakuOid.keyword").value(oid.toString))._toQuery())
    val johtaaTutkintoonQuery = searchParams.johtaaTutkintoon.map(johtaaTutkintoon =>
      TermQuery.of(m => m.field("johtaaTutkintoon").value(johtaaTutkintoon))._toQuery()
    )
    val alkamiskausiQuery = searchParams.alkamiskausi.map(kausi =>
      TermQuery.of(m => m.field("paateltyAlkamiskausi.kausiUri").value(kausi))._toQuery()
    )
    val alkamisvuosiQuery = searchParams.alkamisvuosi.map(vuosi =>
      TermQuery.of(m => m.field("paateltyAlkamiskausi.vuosi").value(vuosi))._toQuery()
    )
              ...
              .map(oid =>
                QueryBuilders.bool
                  .should(
                    TermsQuery
                      .of(m =>
                        m.field("jarjestyspaikka.oid.keyword")
                          .terms(
                            new TermsQueryField.Builder()
                    ...
                    QueryBuilders
                      ...
                      .must(
                        TermsQuery
                          .of(m =>
                            m.field("toteutus.tarjoajat.oid.keyword")
                              .terms(
                                new TermsQueryField.Builder()
                                  .value(List(FieldValue.of(oid.toString())).asJava)
                                  .build()
